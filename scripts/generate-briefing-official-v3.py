...
"""
香港簡報生成腳本（官方數據版 v3）
使用香港政府官方 API：data.gov.hk、香港天文台、運輸署特別交通消息、城巴 ETA
"""
...
LOG_FILE = os.path.join(WORKSPACE, "logs/briefing-official-v3.log")
...
class OfficialBriefingGeneratorV3:
    """官方數據簡報生成器 v3（包含城巴 ETA 數據）"""
    ...
    def fetch_ctb_bus_eta(self):
        ...
        try:
            ...
            popular_routes = [
                ...
                {"route": "10", "name": "10號線 (北角碼頭 ↔ 堅尼地城)"},
                {"route": "101", "name": "101號線 (觀塘 ↔ 堅尼地城)"},
                {"route": "970", "name": "970號線 (蘇屋 ↔ 香港仔)"}
            ]
            
            eta_results = []
            
            for route_info in popular_routes[:3]:  # 只檢查前3條路線
                route = route_info["route"]
                route_name = route_info["name"]
                
                try:
                    # 城巴 ETA API
                    url = f"https://rt.data.gov.hk/v1/transport/citybus-nwfb/eta/CTB/{route}/1"
                    response = requests.get(url, timeout=5)
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get("data"):
                            ...
                            if etas:
                                ...
                                for eta in etas:
                                    ...
                                    if eta_time:
                                        ...
                                        try:
                                            ...
                                            if time_diff >= 0:  # 未來時間
                                                if nearest_eta is None or time_diff < nearest_eta["minutes"]:
                                ...
                                if nearest_eta:
                                    minutes = int(nearest_eta["minutes"])
                                    if minutes <= 30:  # 只顯示30分鐘內的班次
                                        eta_results.append({
                                            "route": route_name,
                                            "minutes": minutes,
                                            "remark": nearest_eta["remark"]
                                        })
                    
                except Exception as e:
                    log(f"獲取路線 {route} ETA 異常: {str(e)}")
                    continue
            
            log(f"找到 {len(eta_results)} 條城巴 ETA 數據")
            return eta_results[:3]  # 返回最多3條
            
        except Exception as e:
            log(f"獲取城巴 ETA 異常: {str(e)}")
    ...
    def generate_briefing(self):
        ...
        # 城巴 ETA 數據
        bus_etas = self.fetch_ctb_bus_eta()
...
---
📡 數據來源：香港天文台、運輸署特別交通消息、data.gov.hk 城巴 ETA、港鐵、Google 新聞
⏰ 天氣數據更新：{update_str}
🔗 官方網站：data.gov.hk、hko.gov.hk、td.gov.hk、mtr.com.hk"""
        
        log(f"官方數據簡報 v3 生成完成，長度: {len(briefing)} 字符")
        return briefing

def main():
    """主函數"""
    try:
        generator = OfficialBriefingGeneratorV3()
        briefing = generator.generate_briefing()
        print(briefing)
        return 0
    except Exception as e:
        log(f"❌ 生成官方數據簡報 v3 時出錯: {str(e)}")
        
        # 緊急備用：使用 v2 官方版
        try:
            from generate_briefing_official_v2 import OfficialBriefingGeneratorV2
            v2_generator = OfficialBriefingGeneratorV2()
            backup = v2_generator.generate_briefing()
            print(backup)
            log("✅ 使用官方版 v2 備用簡報成功")
            return 0
        except:
