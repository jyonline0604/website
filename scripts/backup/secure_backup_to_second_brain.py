#!/usr/bin/env python3
"""
安全備份腳本 - 備份到 Second-brain GitHub repository
備份目錄：my-novel（小說網站）
"""
...
# 修正：使用正確的目錄名稱
INCLUDE_DIRS = ["my-novel", "memory", "scripts"]
EXCLUDE_PATTERNS = [".env", ".git", ".backup-pass", "/logs/", "node_modules", "__pycache__", ".pyc", ".DS_Store", "backup_before_fix"]
...
def upload_to_github(encrypted_file: Path):
    ...
    if not second_brain_dir.exists():
        print(f"📁 Clone Second-brain repository...")
        subprocess.run(["git", "clone", repo_url, str(second_brain_dir)], check=True, capture_output=True)
    else:
        print(f"📁 Second-brain 已存在，拉取最新...")
        subprocess.run(["git", "-C", str(second_brain_dir), "pull"], check=True, capture_output=True)
    
    dest = second_brain_dir / encrypted_file.name
    shutil.copy2(encrypted_file, dest)
    
    for c in [["add", encrypted_file.name], 
              ["commit", "-m", f"Backup: {encrypted_file.name} ({datetime.now().strftime('%Y-%m-%d %H:%M')})"], 
              ["push"]]:
        subprocess.run(["git", "-C", str(second_brain_dir)] + c, check=True, capture_output=True)
    
    print(f"✅ 已上传: {encrypted_file.name}")

def main():
    print("=" * 50)
    print("安全備份系統 - 備份到 Second-brain")
    print("=" * 50)
