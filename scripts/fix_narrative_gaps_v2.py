#!/usr/bin/env python3
"""修復敘事斷層 v2 - 用DeepSeek生成連接段落 + 跳過已修復"""
...
SKIP_FILE = os.path.join(WORKSPACE, '.fixed_pairs.txt')

def tokenize(text):
    chars = re.findall(r'[\u4e00-\u9fff]{2,4}', text)
    return set(chars)

def get_discontinuity_pairs(top_n=15):
    skip_set = set()
