#!/usr/bin/env python3
"""
香港簡報生成腳本（官方數據版 v4 - 完整版）
使用香港政府官方 API：data.gov.hk、香港天文台、運輸署特別交通消息、城巴 ETA、港鐵下一班列車
"""
...
LOG_FILE = os.path.join(WORKSPACE, "logs/briefing-official-v4.log")
...
class OfficialBriefingGeneratorV4:
    """官方數據簡報生成器 v4（包含港鐵下一班列車數據）"""
    ...
    def fetch_mtr_next_train(self):
        ...
        try:
            ...
            for station_info in popular_stations[:3]:  # 只檢查前3個站點
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get("status") == 1 and data.get("data"):
                            ...
                            if station_data:
                                ...
                                # 合併兩個方向的列車
                                all_trains = []
                                for train in up_trains[:2]:  # 每個方向取前2班
                                    train["direction"] = "上行"
                                ...
                                # 獲取最近的列車
                                if all_trains:
                                    nearest_train = all_trains[0]
                                    train_time = nearest_train.get("time", "")
                                    dest = nearest_train.get("dest", "")
                                    ttnt = nearest_train.get("ttnt", "")
                                    direction = nearest_train.get("direction", "")
                                    ...
                                    try:
                                        ...
                                        # 計算分鐘差
                                        time_diff = (train_dt - now).total_seconds() / 60
                                        ...
                                        if time_diff >= 0:  # 未來時間
                                            ...
                                            next_trains.append({
                                                ...
                                                "direction": direction,
                                                "platform": nearest_train.get("plat", "1")
                                            })
                                    except:
                                        continue
    ...
    def fetch_ctb_bus_eta(self):
        ...
        try:
            ...
            for route_info in popular_routes[:2]:  # 只檢查前2條路線
